...
void UPMac::send_data_schedule()
{
	...
	while(send_packet_num > 0 && iTemp < PACKET_NUM_)
	{
		if(send_seq[iTemp])
		{
			...
			printf("ssssssssssssssssSendTimeTemp = %f",SendTimeTemp);
...
void UPMac::addAnnPacket(Packet* pkt)
{
	...
	int iTemp = 0;
		...
		if(last_send_seqence[iTemp] != -1)
		{
			...
			{
				//printf("not recving this packet\n");
			}
		}
	...
	if(iNodeInfoNum == 0 && node_->nodeid())
	{
	...
	{	
		...
		last_send_seqence[iTemp] = ifill_seq_res;
		printf("tttttttttttttttttthe %dth seq is %d\n",iTemp,ifill_seq_res);
		iTemp++;
	}
	hdr_cmn*  cmh = HDR_CMN(pkt);	
...
void UPMac::recvAnn(Packet* p)
{
	...
	if(node_phase == 0)
	{
		...
		printf("in recvAnn,the NEW last_slot_time is %f\n",last_slot_time);
		Scheduler& s = Scheduler::instance(); 
