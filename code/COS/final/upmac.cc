...
void FinishSendAnnHandler::handle(Event* p)
{
	...
	if(mac_->sendReq)
	{
		...
		s.schedule(&(mac_->send_req_handler),&(mac_->send_req_event),0.1);
	}
}
...
void UPMac::send_ann()
{
	...
	iTemp =Random::random() % 5;
	printf("ppppppppppppppppppppppppppppppppiTemp = %d\n",iTemp);
	s.schedule(&send_ann_handler,&send_ann_event,slot_length + iTemp * txtime);
}
...
//	printf("next_free_time is %f,NOW is %f,transTime is %f,this is node %d\n",next_free_time,NOW,transTime,node_->nodeid());
	...
	{			//this means collision happened
		...
		if(next_free_time < NOW + transTime)
		{
			next_free_time = NOW + transTime;
		}
	...
	Scheduler& s = Scheduler::instance();
	s.schedule(&fin_tran_handler,&fin_tran_event,transTime);	
}
...
int UPMac::getProcessCase(Packet* p)
{
	...
	//	printf("\ntest in getProcessCase,last_slot_time = %f,NOW = %f\n\n",last_slot_time,NOW);
	}
