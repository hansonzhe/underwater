...
void ack_link::printAck()
{
	printf("qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq\n");
	printf("\ntarget_id = %d,ack_seq = %d\n",target_id,ack_seq);
	ack_link* aTemp = this->next;
	while(aTemp != NULL)
	{
		printf("target_id = %d,ack_seq = %d\n",aTemp->target_id,ack_seq);
		aTemp = aTemp->next;
	}
}
...
void UPMac::recvData(Packet* p)
{
	...
	aTemp->target_id = hdr->target_id.addr_;
