...
void RecvUnlockHandler::handler(Event* p)
{
	Packet* pkt = (Packet*)p;
...
void SendUnlockHandler::handler(Event* p)
{
	Packet* pkt = (Packet*)p;
        hdr_upmac* upmac = HDR_UPMAC(pkt);                                 
        hdr_cmn*  cmh = HDR_CMN(pkt);
...
{
	if((upmac_stat == UPMAC_SENDER && sender_stat == SENDER_PHASE2) || (upmac_stat == UPMAC_ALOHA_IDLE))
	{
	        Packet* pkt =Packet::alloc();                                      
        	hdr_upmac* upmac = HDR_UPMAC(pkt);                                 
	        hdr_cmn*  cmh = HDR_CMN(pkt);   
		hdr_uwvb* hdr = HDR_UWVB(pkt);
		if(node_->nodeid() != hdr->target_id.addr)
	        //add common packet head and upmac packet head                     
	        cmh->next_hop()=MAC_BROADCAST;
		cmh->ts_ = NOW;
	        cmh->direction()=hdr_cmn::DOWN;
