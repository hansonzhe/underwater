...
NodeInfo* NodeInfoBuffer::getNodeInfo()
{
	...
	while(nTemp != NULL)
	{
		if(dTemp == nTemp->delay && false ==(nTemp->mark) )
		{
			nTemp->mark = true;
			return nTemp;
		}
...
void FinishSendDataHandler::handle(Event* p)
{
	...
	mac_->sender_stat = SENDER_PHASE1;
	UnderwaterSensorNode* n=(UnderwaterSensorNode*) mac_->getNode();
	n->SetTransmissionStatus(IDLE);
	mac_->send_ann();
}
...
void UPMac::send_ann()
{
	...
	{
		if(isSendLocked() || isRecvLocked())
		{
			return;
		}
		enter_recv_ = false;
		s.cancel(&enter_receiver_event);
	...
	printf("111111111111111111111111111111111111111\n");
		...
		if(!enter_recv_)
		{
			s.schedule(&send_ann_handler,&enter_receiver_event,5 * (max_tran_latency +3 * data_time_length));
			enter_recv_ = true;
		}
	}
...
void UPMac::addAnnPacket(Packet* pkt)
{
	...
	double dTemp = NOW + nTemp->delay * 2 + guard_time;
	ann_schedule* aTemp;
	if(upmac_stat == UPMAC_SENDER && sender_stat == SENDER_PHASE3)	
	{
		while(nTemp)
	...
	while(nTemp)
	{
		...
		printf("target_id = %d,pk_num = %d,start time = %f,delay = %f,req_num = %d\n",aTemp->target_id,aTemp->packet_num,dTemp,nTemp->delay,nTemp->req_num);
...
void UPMac::send_data(Packet* p)
{
	if(is_sending)
	{
		printf("error in send data! the node %d is sending now!\n",node_->nodeid());
	}
	Packet* pkt;
	...
	while(s_head != NULL)
	{
		if(s_head->target_id == node_->nodeid())
		{
			...
			if(packet_tosend == 0)
			{
				printf("\nerror in send_data!no packet to send \n");
				return;
			}
			break;
		}
	...
	{
		if(data_start_time > -0.1)
		{
			//means tolerable error
			data_start_time = 0;
		}
		else
		{
			//means intolerable error
			return;
		}
	}
	s.schedule(&send_ann_handler,p->copy(),data_start_time);//send pkt 9
	...
	pkt =Packet::alloc();
        s.schedule(&fin_send_data_handler,pkt,data_start_time);
	if(!enter_recv_)
	{
		s.schedule(&send_ann_handler,&enter_receiver_event,6 * (max_tran_latency +3 * data_time_length));
		enter_recv_ = true;
	}
}
...
void UPMac::send_syn(int addr)
{
	...
	upmac->ts3 = NOW;
 	upmac->reqNum = node_info_buf->getReqNum(addr);
...
void UPMac::aloha_send_data()
{
	...
	{
		printf("\nerror in aloha send data,no packet!\n");
		return;
	}
	...
	printf("\naloha send data: node = %d,reqNum = %d,over addr = %d,overhear req = %d\n",node_->nodeid(),upmac->reqNum,upmac->overhear_recv_addr,upmac->overhear_req_num);
...
//	printf("upmac:Add a packet in the buffer: node %d at %f \n",index_,NOW);
	...
	iTemp =Random::random() % 10 + 1;
//	printf("test in TxProcess iTemp = %d\n",iTemp);
	if(started_)
	{
		return;
	}
		s.schedule(&send_ann_handler,&enter_receiver_event,iTemp * (max_tran_latency +3 * data_time_length));
		enter_recv_ = true;
		started_ = true;
	return;
}
...
void UPMac::recvSyn()
{
	...
	if(p_type == 6)
	{
		...
		printf("stat = %d,sender_stat = %d\n",upmac_stat,sender_stat);
	}
	if(upmac_stat == UPMAC_ALOHA_IDLE && p_type == 6)
	{
	printf("777777777777777777777777777777777777777777\n");
		time_stamp1 = upmac->ts1;
		time_stamp2 = NOW - syn_time_length;
		upmac_stat = UPMAC_SENDER;
	...
	if(upmac_stat == UPMAC_RECEIVER)
	{
		if(p_type == 7 && recv_stat == PHASE2)
		{
			...
			double delay = (NOW - upmac->ts1 - (upmac->ts3 - upmac->ts2) ) / 2 + syn_time_length;
			node_info_buf->updateNode(addr, delay,upmac->reqNum);
		}
		else
		{
			//else do nothing
	...
	if(upmac_stat == UPMAC_SENDER && sender_stat == SENDER_PHASE2)
	{
		...
		if(p_type == 8)
		{
	printf("99999999999999999999999999999999999999\n");
			send_data(p->copy());
		}
		if(p_type == 9)
		{
